...
Bsdf *make_glass_material(const Spectrum &Kr, const Spectrum &Kt, float u_roughness, float v_roughness, float index, bool remap_roughness)
{
    Bsdf *bsdf = new Bsdf();
    ...
    bool is_specular = (u_roughness == 0) && (v_roughness == 0);
    bool allow_multiple_lobes = false; // TODO: configurable?
    if (is_specular && allow_multiple_lobes)
    {
        bsdf->add(new FresnelSpecular(Kr, Kt, 1, eta));
        return bsdf;
    }
    ...
    if (!is_specular)
        distribution = new TrowbridgeReitzDistribution(u_roughness, v_roughness);
    ...
    if (!Kr.is_black())
    {
        ...
        if (is_specular)
            ...
            bsdf->add(new MicrofacetReflection(Kr, distribution, fresnel));
    }

    if (!Kt.is_black())
    {
        if (is_specular)
            bsdf->add(new SpecularTransmission(Kt, 1, eta));
        else
            bsdf->add(new MicrofacetTransmission(Kt, distribution, 1, eta));
    }
